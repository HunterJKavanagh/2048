...
func update(input_array: Array) -> Array:
	...
	if on:
		...
		for n in range(layer_data[-1]):
			output_values.push_back(network[counter + n].value)
			pass
	
	return output_values


func mutate(type = "null"):
	var i1: int = floor(rand_range(0, weights.size()))
	if type == "null":
		weights[i1] = weights[i1] + (rand_range(-1, 1) / 5)
	elif type == "rand":
		weights[i1] = rand_range(-1, 1)
